...
const updateUserprofilePicture = asyncHandler(async (req, res) => {
  const avatarLocalPath = req.file?.path; // Path of the uploaded file on the server
  ...
  // Update user's avatar with the local file path
  const user = await User.findByIdAndUpdate(
    userId,
    { $set: { avatar: avatarLocalPath } }, // Save the local path
    { new: true }
  ).select("-password");
...
export const updateProfile = asyncHandler(async (req, res) => {
  const { fullName, email, mobile, gender, password, country,userId } = req.body;
